...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.window_size = window_size
        self.shift_size = [int(i//2) for i in window_size]
        self.depth = depth
...
class SwinTransformer(nn.Module):
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(8 * embed_dim / 2**i_layer), depth=depths[i_layer],
                num_heads=num_heads[i_layer], window_size=window_size, minpatch=True if i_layer==0 else False,
    ...
    def forward(self, x, out):
        ...
        x = self.norm512(x)
        x, Wh, Ww = self.layers[0](x, Wh, Ww)

        x = x.view(-1, Wh, Ww, 8*self.embed_dim).permute(0, 3, 1, 2).contiguous()

        x = nn.PixelShuffle(2)(x)
        x = torch.cat([x, out[2]], dim=1).flatten(2).transpose(1, 2)
        x = self.redduce1(x)
        ...
        x = x.view(-1, Wh, Ww, 4*self.embed_dim).permute(0, 3, 1, 2).contiguous()

        x = nn.PixelShuffle(2)(x)
        x = torch.cat([x, out[1]], dim=1).flatten(2).transpose(1, 2)
        x = self.redduce2(x)
        x = self.norm128(x)
        x, Wh, Ww = self.layers[2](x, 2*Wh, 2*Ww)

        x = x.view(-1, Wh, Ww, 2*self.embed_dim).permute(0, 3, 1, 2).contiguous()

        x = nn.PixelShuffle(2)(x)
        x = torch.cat([x, out[0]], dim=1).flatten(2).transpose(1, 2)
        x = self.redduce3(x)
        x = self.norm64(x)
        x, Wh, Ww = self.layers[3](x, 2*Wh, 2*Ww)

        B, n_patch, hidden = x.size()
        h, w = int(64), int(n_patch/64)
        x = x.permute(0, 2, 1)
